...
// Define the structure of financial data returned by the analyzer
...
export function FinanceAnalysis({ data, config = {} }: FinanceAnalysisProps) {
    ...
    try {
      ...
      return {
        financialData: {
          ...
          expenses: (result as any).expenses || 0,
          trends: result.trends || [],
          availableMetrics: (result as any).availableMetrics || [],
          dataQuality: {
  ...
  const defaultMetrics: Record<string, Metric> = {
    roi: {
      ...
      tooltip: "Measures the profitability of investments relative to their cost"
    },
    profitMargin: {
      label: "Profit Margin",
      ...
      tooltip: "Shows what percentage of revenue has turned into profit"
    },
    cashFlow: {
      label: "Cash Flow",
      ...
      tooltip: "Net amount of cash moving in and out of business"
    },
    revenue: {
      label: "Revenue",
      ...
      formatter: currencyFormatter,
      icon: <DollarSign className="w-4 h-4" />,
      tooltip: "Total income generated from normal business operations"
    },
    expenses: {
      label: "Expenses",
      ...
      formatter: currencyFormatter,
      icon: <DollarSign className="w-4 h-4" />,
      tooltip: "Total costs incurred in the process of generating revenue"
    }
  };

  // Determine which metrics to display
  const activeMetrics = metricCards.metrics 
    ? Object.entries(metricCards.metrics)
        .filter(([_, isActive]) => isActive)
        .map(([key]) => defaultMetrics[key])
        .filter(Boolean)
    : Object.values(defaultMetrics);
